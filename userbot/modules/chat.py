...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if r_msg.media:
            ...
            await edit_or_reply(event, "Gʀᴏᴜᴘ ID : `{}`\nFʀᴏᴍ Usᴇʀ ID : `{}`\nID Bot File API: `{}`".format(str(event.chat_id), str(r_msg.from_id), bot_api_file_id))
        else:
            await edit_or_reply(event, "Gʀᴏᴜᴘ ID : `{}`\nFʀᴏᴍ Usᴇʀ ID : `{}`".format(str(event.chat_id), str(r_msg.from_id)))
    else:
        await edit_or_reply(event, "Gʀᴏᴜᴘ ID : `{}`".format(str(event.chat_id)))


@Zhu_cmd(pattern="link(?: |$)(.*)")
...
@Zhu_cmd(pattern="kickme$")
async def kickme(leave):
    """ Basically it's .kickme command """
    await edit_or_reply(leave, f"**{ALIVE_NAME} Lᴇᴀᴠᴇ ✔")
    await leave.client.kick_participant(leave.chat_id, 'me')


@Zhu_cmd(pattern="unmutechat$")
async def unmute_chat(unm_e):
    """ For .unmutechat command, unmute a muted chat. """
    try:
        from userbot.modules.sql_helper.keep_read_sql import unkread
    except AttributeError:
        await edit_or_reply(unm_e, 'Rᴜɴɴɪɴɢ ᴏɴ Nᴏɴ SQL Mᴏᴅᴇ')
        return
    unkread(str(unm_e.chat_id))
    await edit_delete(unm_e, "Uɴᴍᴜᴛᴇ Gʀᴏᴜᴘ ✔")


@Zhu_cmd(pattern="mutechat$")
async def mute_chat(mute_e):
    """ For .mutechat command, mute any chat. """
    try:
        from userbot.modules.sql_helper.keep_read_sql import kread
    except AttributeError:
        await edit_or_reply(mute_e, "Rᴜɴɪɴɢ ɴᴏɴ SQL ᴍᴏᴅᴇ")
        return
    await edit_or_reply(mute_e, str(mute_e.chat_id))
    kread(str(mute_e.chat_id))
    await edit_delete(mute_e, "Gʀᴏᴜᴘ Mᴜᴛᴇᴅ ✔")
    if BOTLOG:
        await mute_e.client.send_message(
            BOTLOG_CHATID,
            str(mute_e.chat_id) + " Mᴜᴛᴇᴅ ✔")
...
@Zhu_cmd(pattern="regexninja (on|off)$")
...
async def fetch_info(chat, event):
    ...
    caption = "Gʀᴏᴜᴘ Iɴғᴏʀᴍᴀᴛɪᴏɴ :\n"
    caption += f"ID: <code>{chat_obj_info.id}</code>\n"
    if chat_title is not None:
        caption += f"{chat_type} Nᴀᴍᴇ: {chat_title}\n"
    if former_title is not None:  # Meant is the very first title
        caption += f"Oʟᴅ Nᴀᴍᴇ: {former_title}\n"
    if username is not None:
        caption += f"{chat_type} Tʏᴘᴇ: Pᴜʙʟɪᴋ\n"
        caption += f"Link: {username}\n"
    else:
        caption += f"{chat_type} Tʏᴘᴇ: Pʀɪᴠᴀᴛᴇ\n"
    if creator_username is not None:
        caption += f"Cʀᴇᴀᴛᴏʀ: {creator_username}\n"
    elif creator_valid:
        caption += f"Cʀᴇᴀᴛᴏʀ: <a href=\"tg://user?id={creator_id}\">{creator_firstname}</a>\n"
    if created is not None:
        caption += f"Cʀᴇᴀᴛᴇᴅ ᴅᴀᴛᴇ: <code>{created.date().strftime('%b %d, %Y')} - {created.time()}</code>\n"
    else:
        caption += f"Cʀᴇᴀᴛᴇᴅ ᴅᴀᴛᴇ: <code>{chat_obj_info.date.date().strftime('%b %d, %Y')} - {chat_obj_info.date.time()}</code> {warn_emoji}\n"
    caption += f"Dᴀᴛᴀ ᴄᴇɴᴛʀᴇ ID: {dc_id}\n"
    if exp_count is not None:
        chat_level = int((1 + sqrt(1 + 7 * exp_count / 14)) / 2)
        caption += f"{chat_type} Lᴇᴠᴇʟ: <code>{chat_level}</code>\n"
    if messages_viewable is not None:
        caption += f"Vɪᴇᴡᴀʙʟᴇ ᴍᴇssᴀɢᴇs: <code>{messages_viewable}</code>\n"
    if messages_sent:
        caption += f"Mᴇssᴀɢᴇ sᴇɴᴅ: <code>{messages_sent}</code>\n"
    elif messages_sent_alt:
        caption += f"Mᴇssᴀɢᴇ sᴇɴᴅ: <code>{messages_sent_alt}</code> {warn_emoji}\n"
    if members is not None:
        caption += f"Mᴇᴍʙᴇʀ: <code>{members}</code>\n"
    if admins is not None:
        caption += f"Aᴅᴍɪɴ: <code>{admins}</code>\n"
    if bots_list:
        caption += f"Bᴏᴛ: <code>{bots}</code>\n"
    if members_online:
        caption += f"Usᴇʀ Oɴʟɪɴᴇ: <code>{members_online}</code>\n"
    if restrcited_users is not None:
        caption += f"Rᴇsᴛʀᴄɪᴛᴇᴅ ᴜsᴇʀ: <code>{restrcited_users}</code>\n"
    if banned_users is not None:
        caption += f"Bᴀɴɴᴇᴅ Usᴇʀ: <code>{banned_users}</code>\n"
    if group_stickers is not None:
        caption += f"{chat_type} Sᴛɪᴄᴋᴇʀ: <a href=\"t.me/addstickers/{chat.full_chat.stickerset.short_name}\">{group_stickers}</a>\n"
    caption += "\n"
    if not broadcast:
        caption += f"Sʟᴏᴡ Mᴏᴅᴇ: {slowmode}"
        if hasattr(
                chat_obj_info,
            ...
            caption += "\n\n"
    if not broadcast:
        caption += f"Sᴜᴘᴇʀɢʀᴏᴜᴘ: {supergroup}\n\n"
    if hasattr(chat_obj_info, "Terbatas"):
        caption += f"Rᴇsᴛʀɪᴄᴛᴇᴅ: {restricted}\n"
        if chat_obj_info.restricted:
            caption += f"> Pʟᴀᴛғᴏʀᴍ: {chat_obj_info.restriction_reason[0].platform}\n"
            caption += f"> Rᴇᴀsᴏɴ: {chat_obj_info.restriction_reason[0].reason}\n"
            caption += f"> Tᴇxᴛ: {chat_obj_info.restriction_reason[0].text}\n\n"
        else:
            caption += "\n"
    if hasattr(chat_obj_info, "scam") and chat_obj_info.scam:
        caption += "Scam: <b>Yes</b>\n\n"
    if hasattr(chat_obj_info, "verified"):
        caption += f"Tᴇʟᴇɢʀᴀᴍ Vᴇʀɪᴠɪᴇᴅ: {verified}\n\n"
    if description:
        caption += f"Dᴇsᴋʀɪᴘᴛɪᴏɴ: \n<code>{description}</code>\n"
    return caption


@Zhu_cmd(pattern="invite(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    to_add_users = event.pattern_match.group(1)
    if event.is_private:
        await edit_or_reply(event, "Usᴇ Cᴏᴍᴍᴀɴᴅ ɪɴ ɢʀᴏᴜᴘ Oɴʟʏ ")
    else:
        if not event.is_channel and event.is_group:
            ...
            await edit_or_reply(event, "Iɴᴠɪᴛᴇ Sᴜᴄᴄᴇss ✔")
        else:
...
CMD_HELP.update({
    "chat":
    f"⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}getid`\
\n✗ Fᴜɴɢsɪᴏɴ : Dapatkan ID dari media Telegram mana pun, atau pengguna mana pun\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}getbot`\
\n✗ Fᴜɴɢsɪᴏɴ : Dapatkan Bot dalam obrolan apa pun.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}logit`\
\n✗ Fᴜɴɢsɪᴏɴ : Meneruskan pesan yang telah Anda balas di grup log bot Anda.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}exit`\
\n✗ Fᴜɴɢsɪᴏɴ : Keluar dari grup.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}unmutechat`\
\n✗ Fᴜɴɢsɪᴏɴ : Membuka obrolan yang dibisukan.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}mutechat`\
\n✗ Fᴜɴɢsɪᴏɴ : Memungkinkan Anda membisukan obrolan apa pun.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}link` <username/userid>: <opsional teks> (atau) balas pesan seseorang dengan .link <teks opsional>\
\n✗ Fᴜɴɢsɪᴏɴ : Buat tautan permanen ke profil pengguna dengan teks ubahsuaian opsional.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}regexninja` enable/disabled\
\n✗ Fᴜɴɢsɪᴏɴ : Mengaktifkan/menonaktifkan modul ninja regex secara global.\
\nModul Regex Ninja membantu menghapus pesan pemicu bot regex.\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}chatinfo [opsional: <reply/tag/chat id/invite link>]`\
\n✗ Fᴜɴɢsɪᴏɴ : Mendapatkan info obrolan. Beberapa info mungkin dibatasi karena izin yang hilang..\
\n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}invite` \
\n✗ Fᴜɴɢsɪᴏɴ : Menambahkan pengguna ke obrolan, bukan ke pesan pribadi. "
})
