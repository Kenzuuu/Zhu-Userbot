...
@Zhu_cmd(pattern="bersihkanbotfilter (.*)")
...
CMD_HELP.update({
    "filter":
    f"⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}filters`\
    \n✗ Fᴜɴɢsɪᴏɴ : Melihat filter userbot yang aktif di obrolan.\
    \n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}filter` <keyword> <balasan> atau balas ke pesan ketik {cmd}filter <keyword>\
    \n✗ Fᴜɴɢsɪᴏɴ : Membuat filter di obrolan.\
    \nBot Akan Membalas Jika Ada Yang Menyebut 'keyword' yang dibuat.\
    \nBisa dipake ke media/sticker/vn/file.\
    \n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}stop` <keyword>\
    \n✗ Fᴜɴɢsɪᴏɴ : Untuk Nonaktifkan Filter.\
    \n\n⦿ Cᴏᴍᴍᴀɴᴅ : `{cmd}bersihkanbotfilter` <marie/rose>\
    \n✗ Fᴜɴɢsɪᴏɴ : Menghapus semua filter yang ada di bot grup (Saat ini bot yang didukung: Marie, Rose.) dalam obrolan."
})
