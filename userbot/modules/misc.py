...
@Zhu_cmd(pattern="shutdown$")
async def killdabot(event):
    """ For .shutdown command, shut the bot down."""
    await edit_or_reply(event, "Shutdown. . .")
...
@Zhu_cmd(pattern="restart$")
async def killdabot(event):
    await edit_or_reply(event, "Restarting. . .")
...
@Zhu_cmd(pattern="string$")
async def repo_is_here(wannasee):
    """For .repo command, just returns the repo URL."""
    await edit_or_reply(wannasee,
                        f"➣ **GET STRING SESSION VIA BOT    :** [KLIK DISINI](https://t.me/ZhuXProtectronBot)\n"
                        )
...
CMD_HELP.update({
    ...
    \n↳ : Menampilan link Repository Userbot.",
    f"string": "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `{cmd}string`\
    \n↳: Menampilkan link String Userbot",
