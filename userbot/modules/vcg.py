...
async def get_call(Zhu):
    kyy = await Zhu.client(getchat(Zhu.chat_id))
    await Zhu.client(getvc(Zhu.full_chat.call, limit=1))
    return hehe.call
...
@Zhu_cmd(pattern="vcinvite")
async def _(Zhu):
    await edit_or_reply(Zhu, "`Sedang Menginvite Member...`")
    users = []
    z = 0
    async for x in Zhu.client.iter_participants(Zhu.chat_id):
        if not x.bot:
            users.append(x.id)
    hmm = list(user_list(users, 6))
    for p in hmm:
        try:
            await Zhu.client(invitetovc(call=await get_call(Zhu), users=p))
            z += 6
        except BaseException:
            pass
    await edit_or_reply(Zhu, f"`Menginvite {z} Member`")


@Zhu_cmd(pattern="vctitle(?: |$)(.*)")
